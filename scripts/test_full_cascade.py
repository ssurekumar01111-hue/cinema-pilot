...
from shared.cascade_dispatch import dispatch_triggered_agents


_RESULT_FIELDS: dict[str, tuple[str, ...]] = {
    "budget": ("budget_line_id", "amount", "reason"),
    "location": ("risk_flag_id", "location_id", "severity", "description"),
    "schedule": ("schedule_block_id", "day_index", "duration_minutes", "constraints"),
    "storyboard": ("storyboard_id", "gs_uri", "signed_url"),
    "music": ("music_cue_id", "status", "gs_uri", "signed_url"),
}


def _print_agent_result(agent_name: str, result: Any) -> None:
    """Keep the full-cascade console output useful after dynamic dispatch."""
    if isinstance(result, list):
        print(f"  Result records: {len(result)}")
        for record in result:
            if isinstance(record, dict):
                print(
                    "  - "
                    f"{record.get('risk_flag_id')} | severity={record.get('severity')} | "
    ...
    for field in _RESULT_FIELDS.get(agent_name, ()):
        if field in result:
            print(f"  {field}: {result[field]}")


def _run_triggered_agents(
    ...
    *,
    ...
    """Run exactly the downstream agents selected by Change Detection.

    The demo previously called all six relocation agents directly. Keeping the
    dispatch table here makes the execution path follow the audited routing
    decision, while remaining explicit about the arguments each agent needs.
    """
    handlers: dict[str, Callable[[], Any]] = {
        "budget": lambda: recalculate_budget(scene_id, cascade_id=cascade_id),
        "location": lambda: assess_location(scene_id, cascade_id=cascade_id),
        "risk": lambda: mitigate_risks(location_id, cascade_id=cascade_id),
        "schedule": lambda: reschedule_shoot(scene_id, cascade_id=cascade_id),
        "storyboard": lambda: generate_storyboard(scene_id, cascade_id=cascade_id),
        "music": lambda: generate_music_cue(scene_id, cascade_id=cascade_id),
    }

    def with_progress(agent_name: str, handler: Callable[[], Any]) -> Callable[[], Any]:
        def run() -> Any:
            print(f"\n--- {agent_name.title()} Agent ---")
            started_at = time.time()
            result = handler()
            print(f"Completed in {time.time() - started_at:.2f}s.")
            _print_agent_result(agent_name, result)
            return result

        return run

    progress_handlers = {
        agent_name: with_progress(agent_name, handler)
        for agent_name, handler in handlers.items()
    }
    return dispatch_triggered_agents(triggered_agents, progress_handlers)
...
def main():
    ...
    relocation_decision = next(
        (
            result
            for result in cd_results
            if result.get("entity_type") == "scene"
            and result.get("entity_id") == "scene_005"
        ),
        None,
    )
    if relocation_decision is None:
        raise RuntimeError("Change Detection did not return a routing decision for scene_005.")

    triggered_agents = relocation_decision.get("triggered_agents", [])
    if not triggered_agents:
        raise RuntimeError("The scene_005 relocation did not trigger any downstream agents.")

    # -------------------------------------------------------------------------
    # STEP 4 — Run only the agents selected by Change Detection
    # -------------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(f"STEP 4: RUN {len(triggered_agents)} ROUTED AGENT(S)")
    print("=" * 80)
    print(f"Routed agents: {', '.join(triggered_agents)}")
    _run_triggered_agents(
        triggered_agents,
        scene_id="scene_005",
